...
- **Collection** (`/collection`) — grid (default) or list view of owned games, remembered across visits; grid shows each game's cover art, list shows title, player count, average play time, and personal rating in a table. A BGG-linked/not-linked filter combines with search and sort. Client-side search filters by title, categories, mechanics, notes, player count, play time, and rating. Each game has Log Play (with undo toast), Edit, and Delete (with confirmation) actions.
