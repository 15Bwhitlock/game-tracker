...
- [x] Phase 4 — Polish
  ...
  - [x] Suggest page grid view — same list/grid toggle as Collection, own `suggestViewMode` localStorage key, grid is the default (see 2026-09-17 decision log entry)
  ...
  - [x] "Powered by BoardGameGeek" attribution footer, added ahead of ever hosting this publicly (see the 2026-09-17 Open questions note)
  - [x] Aligned several preset category/mechanic names with BGG's actual terms (V14 migration) — see 2026-09-17 decision log entry
  - [x] Wishlist page (`/wishlist`) — browse BGG's trending list or search by name, filter either by category/mechanic, add to a persistent wishlist, remove, or move an entry into the owned collection; new `wishlist_items` table (V15)
...
- **2026-09-17** — **Wishlist page** (`/wishlist`) — games not yet owned, kept for future reference. Prompted by "make a new page called wishlist that lets me browse games i dont have by category." BGG's XML API2 has no "browse by category" endpoint (only name search), so after discussing realistic options landed on combining two real discovery paths: **Browse Trending**, BGG's actual `/xmlapi2/hot?type=boardgame` endpoint (verified live via curl), enriched per-item with a full `/thing` lookup so results are filterable by category/mechanic the same way Suggest's chips work; and **Search by name**, reusing the existing `BggApi.search()`/`details()` flow already built for the add-game form. Both feed the same wishlist. New `Wishlist` entity + `wishlist_items`/`wishlist_categories`/`wishlist_mechanics` tables (V15, mirroring `Game`'s child-table pattern but lighter — no play-tracking columns, most fields nullable since a wishlist entry can be sparse); `WishlistController`/`WishlistService` (`GET`/`POST`/`DELETE /api/wishlist`, `POST /api/wishlist/{id}/move-to-collection` — copies fields into a real `Game`, same "personal fields start empty, fill in later via Edit" pattern as any other BGG import, so no need for a separate pre-filled-form detour). New `GET /api/bgg/hot` on `BggClient`, reusing the existing `BggSearchResponse` DTO directly (the hot list's `<item><name value/><yearpublished value/></item>` shape is identical to search results', once the extra `rank` attribute and `<thumbnail>` element are ignored) — no new XML DTO class needed.
  - Caught two real bugs writing this. **Self-invocation bypassing `@Cacheable`**: `getHotListWithDetails()` called `this.getDetails(id)`/`this.hotList()` directly — Spring's caching proxy only intercepts calls that arrive *through* the proxy, so a same-class call skips it entirely, silently. First symptom was the "cached" second call to `/api/bgg/hot` taking the same ~30s as the cold one. Fixed with the standard `@Lazy` self-injection pattern (a `BggClient self` field, injected as `@Lazy BggClient self` in the constructor to avoid the circular-dependency deadlock a plain self-reference would cause) — internal calls now go through `self.getDetails(...)`/`self.hotList()` instead of `this`, and the second call dropped to ~0.16s. **BGG rate-limiting**: ~50 sequential `/thing` lookups with no pacing 429'd roughly a third of them (discovered by comparing raw response counts before/after — 31–41 of ~50 enriched, depending on run), and since `getDetails()` caches *any* return value including a 429-triggered empty Optional, those failures would otherwise have stuck for the full 24h TTL even though BGG would happily serve them moments later. Fixed with a flat 500ms pace before every actual outbound `/thing` request (in `fetchThingXml`, so it only costs anything on a genuine cache miss — never on a hit, and imperceptible on the existing single-lookup BGG-import path) — brought successful enrichment up to 41/50, with the residual handful still occasionally 429'ing being accepted as reasonable BGG-limit variance rather than chased further.
  - New e2e tests (`wishlist.spec.ts`) needed a dev-server restart mid-session to pick up the brand-new lazy-loaded route/component (Vite's dev server didn't pick up an entirely new route file via HMR the way it does edits to existing ones). Also hit — and fixed — two BGG-search-specific test-writing pitfalls worth remembering: BGG's `/search` can return an *alternate* display name for a hit that differs from the full record's actual primary name (matching a check on `bggId`/count-delta is reliable; matching on displayed text is not), and real BGG titles added during a test don't get the `E2e`-prefix safety net that seeded titles do, so tests that add via search/trending must explicitly clean up in a `try/finally` rather than rely on the describe block's prefix-based `afterEach`. 88 backend + (scoped) 5/5 `wishlist.spec.ts` + 4/4 `app-shell.spec.ts` tests pass.

- **2026-09-17** — **Aligned several preset category/mechanic names with BGG's actual terms.** Prompted by "take a look at the mechanics and categories and let me know if you think i should update them to more closely align with bgg." Rather than guess at BGG's exact current wording (their terms/wiki pages 403 automated fetches — confirmed again this session), diffed the app's presets against the real category/mechanic strings pulled in from this collection's 29 BGG-linked games — a more reliable source than memory. Renamed 5 categories (Party→Party Game, Abstract→Abstract Strategy, Dice Game→Dice, Children's→Children's Game, Word→Word Game) and 5 mechanics (Cooperative Play→Cooperative Game, Solo / Solitaire→Solo / Solitaire Game, Scenario / Mission→Scenario / Mission / Campaign Game, Point-to-Point Movement→Point to Point Movement, Trick Taking→Trick-taking) to match BGG's real wording. Also moved "Deck Building" out of categories entirely — BGG has no such category, only the mechanic (renamed to BGG's actual current name, "Deck, Bag, and Pool Building") — via a category-to-mechanic migration mirroring the existing V7/V8/V10 pattern. New `V14__align_category_mechanic_names_with_bgg.sql` renames existing stored values in place (guarded per-row against an unlikely pre-existing duplicate) so this collection's real data — not just the preset list — updates too; verified against both a fresh Testcontainers Postgres (`GameRepositoryTest`, 3/3) and the actual long-running dev database (schema now at v14, spot-checked live via the API that every renamed value shows the new name and no game retained an old one — including "Face Cards," the one real game that had "Deck Building" as a category, now correctly showing "Deck, Bag, and Pool Building" as a mechanic). The Dictionary page needed no separate changes — it renders directly from the same `GAME_CATEGORIES`/`GAME_MECHANICS` arrays, so it picked up the renames automatically. Deliberately did *not*: reclassify "Bluffing" or "Deduction" back to categories (BGG's own classification) — this app already made the opposite call deliberately in V7/V8, and reversing that wasn't asked for; add the long tail of niche BGG terms found in the real data but absent from the presets (∼20 categories, ∼35 mechanics) — the preset lists are deliberately curated to fit a single browsable toggle-chip grid (see the 2026-05-11 decision log entry), and those terms already work fine via the existing custom-tag path; or touch the three remaining categories/mechanics name overlaps (Hidden Roles, Negotiation, Roll & Write) — the real BGG data shows "Deduction" genuinely double-classified as both a category and a mechanic across different games, which is evidence this is a real BGG pattern, not a bug, for the other three as well.

- **2026-09-17** — **Made grid the default view on the Suggest page** (flipped from the 2026-09-16 decision to keep list as the default there). `suggestViewMode`'s fallback changed from `'list'` to `'grid'` in `suggest-page.ts`; the rest of the toggle/persistence machinery is unchanged. Existing tests that assert against list markup (`<li class="suggestion">`) needed a `test.beforeEach` forcing list view via `localStorage`, matching the pattern Collection's own suite already uses for the same reason. The one test that actually verifies the default and the toggle moved into its own `test.describe` block outside that `beforeEach`'s scope — Playwright's `addInitScript` calls stack and all re-run on every navigation including `page.reload()`, so keeping that test inside the list-forcing describe (or trying to counter it with another `addInitScript` inside the test) either silently pinned it to list regardless of the real default, or wiped out a mid-test view change on the reload it was trying to verify persisted. Verified with the scoped test file only (`suggestions.spec.ts`, 17/17 passing) per the user's instruction to run only tests correlated to a change rather than the full suite by default.
